import pandas as pd
import numpy as np

class WalkForwardValidator:
    @staticmethod
    def validate_robustness(model, df, features, n_slices=3):
        """
        Splits the 6-month data into 3 rolling windows.
        A strategy MUST have a positive Inverted EV in all 3 slices to pass.
        """
        # Calculate split indices for np.array_split equivalent on DataFrame
        n = len(df)
        indices = np.array_split(range(n), n_slices)
        slice_results = []

        for i in range(len(indices)):
            test_slice = df.iloc[indices[i]]
            
            # Predict failures on this slice
            probs = model.predict(test_slice[features])
            
            # Calculate Inverted Returns (Flip the -1.0Rs into +1.0Rs)
            # Simplified: If prob > 0.75, we take the negative of the outcome
            returns = np.where(probs >= 0.75, -test_slice['outcome_r'], 0)
            
            slice_ev = np.mean(returns[returns != 0]) if np.any(returns != 0) else -1
            slice_results.append(slice_ev)

        # Robustness Check: Are all slices profitable?
        is_robust = all(res > 0.05 for res in slice_results)
        print(f"🔄 Walk-Forward EVs: {[round(r, 3) for r in slice_results]} | Robust: {is_robust}")
        return is_robust